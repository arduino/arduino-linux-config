...
func Rebuild(ctx context.Context, reg registry.Registry, cfg config.Configuration, desired Desired, dryRun bool) (string, []string, error) {
	...
	command, err := applier.Apply(ctx, overlays, dryRun)
	if err != nil || dryRun {
		return command, incompatible, err
	}

	for name, state := range desired {
		mount, exist := reg.FindByName(string(name))
		if !exist {
			continue
		}
		if err := status.Update(cfg, mount, state); err != nil {
			return command, incompatible, fmt.Errorf("failed to update status for %s: %w", name, err)
		}
	}
	return command, incompatible, nil
}
