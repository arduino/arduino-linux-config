...
func TestSplitDeviceTrees(t *testing.T) {
	first := fakeDeviceTree("qcom,other")
	second := fakeDeviceTree("arduino,monza")
	combined := append(append([]byte{}, first...), second...)

	deviceTrees, err := splitDeviceTrees(combined)
	require.NoError(t, err)
	require.Equal(t, [][]byte{first, second}, deviceTrees)

	_, err = splitDeviceTrees([]byte("not a device tree"))
	require.Error(t, err)
}
